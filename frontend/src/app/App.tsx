    ...
    try {
      ...
      const message = error.response?.data?.message || 
        (error.code === 'ERR_NETWORK' 
          ? 'Cannot connect to server. Please ensure the backend is running at http://localhost:5000' 
      ...
      auth.getMe()
        ...
        .catch((error) => {
          ...
          if (error.response && error.response.status === 401) {
            console.log('Unauthorized session, clearing token.');
            localStorage.removeItem('token');
          } else if (!error.response) {
            console.warn('Server unreachable (Network Error). Keeping token for future retry.');
          } else {
            console.error('Unexpected error during session check:', error.response.status);
          }
        });
    }
  }, []);
