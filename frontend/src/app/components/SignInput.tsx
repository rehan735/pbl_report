  ...
  const captureSign = async () => {
    ...
    canvas.toBlob(async (blob) => {
      ...
      try {
        ...
        const data: { prediction?: string; confidence?: number } = await response.json();
        ...
        setError(error.message || 'Failed to analyze sign');
      }
          ...
          <div className="bg-indigo-50 rounded-xl p-6 border border-indigo-100">
            ...
            {lastConfidence !== null && (
              <p className="text-sm text-indigo-600 mb-4">
                Last prediction confidence: {(lastConfidence * 100).toFixed(1)}%
              </p>
            )}
